...
def index():
    ...
    if request.method == 'POST':
        ...
        # Get current BTC price and run the trading strategy
        btc_price = get_btc_price()
        capital, portfolio_value, highest_price, message = trading_strategy(
            btc_price, initial_capital, initial_capital, pruning_ratio, stop_loss_threshold, btc_price
        )
...
def backtest():
    ...
    if request.method == 'POST':
        ...
        # Run the backtest
        capital_history, portfolio_history, messages = backtest_strategy(initial_capital, pruning_ratio, stop_loss_threshold)
        
        # Optionally, display some key messages in the UI
        sample_messages = messages[:10]  # Just showing the first 10 messages for simplicity
        return render_template('backtest.html', sample_messages=sample_messages)
